...
- [ ] PostgreSQL 创建生产数据库和用户，配置连接
- [ ] Next.js 项目 `next.config.ts` 配置 `output: 'standalone'`
- [ ] PM2 ecosystem 配置文件，守护 Next.js standalone server (:3000)
- [ ] Nginx 配置：监听 443，SSL 证书（certbot / Let's Encrypt），反向代理到 :3000，静态资源缓存
- [ ] DNS：`mybaicao.com` A 记录指向服务器 IP
- [ ] `.env.production` 配置所有环境变量
